%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%    This function simulates a 2/2 transmitter/receiver
%system using Alamouti scheme %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%errorbits: is the number of errors after which the iterations stop.
%snr: the signal to noise ratio.

function BER=two_rec(snr,errorbits);
 snr=-5:9;
for w=1:length(snr)
...
while tot_err_ml<errorbits                                      %Starting the loop
    ...
    iter=iter+1;                                                 %Counting the iterations
        ...
        for i=1:no_it_x_SNR                                                 %Starting the simulation
           ...
           % H=ones(2,2);                                                   %If u want a AWGN channel, use this!
            ...
            sig = sqrt(0.5/(10.^(snr(w)/10)));                                  %Noise variance 
            ...
            BER(w)=tot_err_ml/(no_it_x_SNR*iter*2)     ;                 %Computing the BER
